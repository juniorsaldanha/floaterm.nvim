...
describe("add_keymap_hide", function()
  it("adds keymap successfully for existing terminal", function()
    local terminal = plugin.register_terminal("test_hide", "<leader>th")
    assert(terminal ~= nil, "terminal should be created")

    local success = plugin.add_keymap_hide("test_hide", "<leader>hq")
    assert(success, "add_keymap_hide should return true for existing terminal")
  end)

  it("fails to add keymap for non-existing terminal", function()
    local success = plugin.add_keymap_hide("non_existing", "<leader>hq")
    assert(not success, "add_keymap_hide should return false for non-existing terminal")
  end)
end)

describe("remove_keymap_hide", function()
  it("removes keymap successfully for existing terminal", function()
    local terminal = plugin.register_terminal("test_remove", "<leader>tr")
    assert(terminal ~= nil, "terminal should be created")

    plugin.add_keymap_hide("test_remove", "<leader>hr")
    local success = plugin.remove_keymap_hide("test_remove", "<leader>hr")
    assert(success, "remove_keymap_hide should return true for existing terminal")
  end)

  it("fails to remove keymap for non-existing terminal", function()
    local success = plugin.remove_keymap_hide("non_existing", "<leader>hr")
    assert(not success, "remove_keymap_hide should return false for non-existing terminal")
  end)
end)
